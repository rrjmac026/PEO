...
<style>
    ...
    /* ── Sub-link (indented filter links) ── */
    .sb-sub-link {
        display: flex;
        align-items: center;
        gap: 10px;
...
<nav class="space-y-1 p-3">
    ...
    <div class="sb-role-badge">
        @if($role === 'site_inspector')
            <i class="fas fa-hard-hat"></i> Site Inspector
        @elseif($role === 'surveyor')
            <i class="fas fa-drafting-compass"></i> Surveyor
        @elseif($role === 'resident_engineer')
            <i class="fas fa-hard-hat"></i> Resident Engineer
        @elseif($role === 'provincial_engineer')
            <i class="fas fa-user-tie"></i> Provincial Engineer
        @endif
    </div>
    ...
    {{-- Status filter sub-links — shown to all roles --}}
    ...
    <a href="{{ route('reviewer.work-requests.index', ['status' => 'accepted']) }}"
       class="sb-sub-link {{ request('status') === 'accepted' ? 'active' : '' }}">
        <span class="sb-dot"></span> Accepted
    </a>
    ...
    <a href="#"
       class="sb-link {{ request()->routeIs('reviewer.concrete-pouring*') && !request('status') ? 'active' : '' }}">
        <span class="sb-icon"><i class="fas fa-fill-drip"></i></span>
        All Pourings
    </a>

    {{-- Concrete Pouring status sub-links --}}
    <a href="#"
       class="sb-sub-link {{ request()->routeIs('reviewer.concrete-pouring*') && request('status') === 'requested' ? 'active' : '' }}">
        <span class="sb-dot"></span> Requested
    </a>

    <a href="#"
       class="sb-sub-link {{ request()->routeIs('reviewer.concrete-pouring*') && request('status') === 'approved' ? 'active' : '' }}">
        <span class="sb-dot"></span> Approved
    </a>

    <a href="#"
       class="sb-sub-link {{ request()->routeIs('reviewer.concrete-pouring*') && request('status') === 'disapproved' ? 'active' : '' }}">
    ...
    @if($role === 'surveyor')
        ...
        <a href="{{ route('reviewer.work-requests.index', ['surveyed' => 'done']) }}"
           class="sb-link {{ request('surveyed') === 'done' ? 'active' : '' }}">
            <span class="sb-icon"><i class="fas fa-map-marked-alt"></i></span>
            Surveyed
        </a>
    @endif
