...
<style>
    ...
    .mo-input.is-invalid, .mo-select.is-invalid, .mo-textarea.is-invalid { border-color: #ef4444; }
    ...
    .mo-file-drop input[type="file"] { position: absolute; inset: 0; opacity: 0; cursor: pointer; }
    ...
    .mo-attach-remove {
        ...
        color: #ef4444; padding: 0; font-size: 11px;
        opacity: .7; transition: opacity .12s;
    }
    .mo-attach-remove:hover { opacity: 1; }
    ...
    .mo-status-banner.scheduled {
        background: #eff6ff; border-color: #93c5fd; color: #1d4ed8;
    }
    .dark .mo-status-banner.scheduled {
        background: rgba(96,165,250,.1); border-color: rgba(96,165,250,.3); color: #60a5fa;
    }
</style>
@endpush
...
@section('content')
    ...
    <div class="mo-breadcrumb">
        ...
        <span class="sep">/</span>
        <a href="{{ route('admin.memos.show', $memo) }}">{{ Str::limit($memo->subject, 40) }}</a>
        <span class="sep">/</span>
        <span>Edit</span>
    </div>
    ...
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="mo-page-title">Edit Memo</h1>
        <p class="mo-page-sub">Update draft or scheduled memo before sending</p>
    </div>

    <!-- Scheduled notice -->
    @if ($memo->status === 'scheduled')
        <div class="mo-status-banner scheduled">
            <i class="fas fa-clock"></i>
            This memo is scheduled to send on <strong>{{ $memo->scheduled_at->format('M d, Y g:i A') }}</strong>.
            Saving changes will update the scheduled version.
        </div>
    @endif

    <form action="{{ route('admin.memos.update', $memo) }}" method="POST" enctype="multipart/form-data" id="memo-form">
        @csrf
        @method('PUT')
                    ...
                    <div class="mo-card-body">
                        ...
                        <div class="mo-form-group">
                            ...
                            <select name="type" class="mo-select {{ $errors->has('type') ? 'is-invalid' : '' }}" required>
                                ...
                                @foreach (\App\Models\Memo::types() as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('type', $memo->type) === $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mo-form-group" style="margin-bottom: 0;">
                            ...
                            <textarea name="body"
                                      ...
                                      required>{{ old('body', $memo->body) }}</textarea>
                    ...
                    <div class="mo-card-body">
                        ...
                        @if ($memo->attachments && count($memo->attachments))
                            <div style="margin-bottom: 14px;">
                                <label class="mo-label" style="margin-bottom:8px;">Existing Attachments</label>
                                <div class="mo-attach-existing">
                                    @foreach ($memo->attachments as $attachment)
                                        <div class="mo-attach-item">
                                            <i class="fas fa-file" style="color:var(--mo-muted);"></i>
                                            <a href="{{ Storage::url($attachment) }}" target="_blank"
                                               style="color:inherit;text-decoration:none;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                                                {{ basename($attachment) }}
                                            </a>
                                            <!-- Remove attachment via separate POST -->
                                            <form action="{{ route('admin.memos.remove-attachment', $memo) }}"
                                                  method="POST" class="inline"
                                                  onsubmit="return confirm('Remove this attachment?')">
                                                @csrf
                                                @method('DELETE')
                                                <input type="hidden" name="path" value="{{ $attachment }}">
                                                <button type="submit" class="mo-attach-remove" title="Remove">
                        ...
                        <div class="mo-file-drop" id="file-drop-zone">
                            ...
                            <div class="mo-file-drop-label">
                                Drag & drop new files or <span>browse</span>
                            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="mo-card">
                    ...
                    <div class="mo-card-body">
                        ...
                        @php $currentScope = old('recipient_scope', $memo->recipient_scope); @endphp
                        ...
                        <div class="mo-form-group">
                            ...
                            <div class="mo-scope-grid">
                                ...
                                <label class="mo-scope-card {{ $currentScope === 'all' ? 'selected' : '' }}">
                                ...
                                <label class="mo-scope-card {{ $currentScope === 'by_role' ? 'selected' : '' }}">
                                    <input type="radio" name="recipient_scope" value="by_role"
                                           {{ $currentScope === 'by_role' ? 'checked' : '' }}
                                           onchange="switchScope(this.value)">
                                    <div>
                                ...
                                <label class="mo-scope-card {{ $currentScope === 'by_department' ? 'selected' : '' }}">
                                    <input type="radio" name="recipient_scope" value="by_department"
                                           {{ $currentScope === 'by_department' ? 'checked' : '' }}
                                           onchange="switchScope(this.value)">
                                    <div>
                                ...
                                <label class="mo-scope-card {{ $currentScope === 'specific' ? 'selected' : '' }}">
                        ...
                        <!-- By Role panel -->
                        <div id="panel-by_role" class="mo-scope-panel {{ $currentScope === 'by_role' ? 'active' : '' }}">
                                ...
                                @foreach ($roles as $role)
                                    <label class="mo-check-item">
                                        <input type="checkbox" name="target_roles[]" value="{{ $role }}"
                                               {{ in_array($role, old('target_roles', $memo->target_roles ?? [])) ? 'checked' : '' }}>
                        ...
                        <!-- By Department panel -->
                        <div id="panel-by_department" class="mo-scope-panel {{ $currentScope === 'by_department' ? 'active' : '' }}">
                            <label class="mo-label">Select Departments</label>
                            <div class="mo-check-grid">
                                @foreach ($departments as $dept)
                                    <label class="mo-check-item">
                                        <input type="checkbox" name="target_departments[]" value="{{ $dept }}"
                                               {{ in_array($dept, old('target_departments', $memo->target_departments ?? [])) ? 'checked' : '' }}>
                                        {{ $dept }}
                                    </label>
                                @endforeach
                            </div>
                        ...
                        <div id="panel-specific" class="mo-scope-panel {{ $currentScope === 'specific' ? 'active' : '' }}">
                                ...
                                @foreach ($users as $user)
                                    <label class="mo-check-item" style="margin-bottom: 4px;" data-name="{{ strtolower($user->name) }}">
                                        <input type="checkbox" name="specific_user_ids[]" value="{{ $user->id }}"
                                               {{ in_array($user->id, old('specific_user_ids', $existingRecipientIds)) ? 'checked' : '' }}>
                                        <span>
                                            {{ $user->name }}
                ...
                <div class="mo-card">
                    ...
                    <div class="mo-card-body">
                        <label class="mo-check-item" style="margin-bottom: 12px;">
                        ...
                        <div id="schedule-section">
                            ...
                            <input type="datetime-local" name="scheduled_at"
                                   value="{{ old('scheduled_at', $memo->scheduled_at ? $memo->scheduled_at->format('Y-m-d\TH:i') : '') }}"
        ...
        <div class="mo-card" style="margin-top: 24px;">
            <div class="mo-action-bar">
                <a href="{{ route('admin.memos.show', $memo) }}" class="mo-btn mo-btn-secondary">
...
<script>
    ...
    // Pre-check schedule toggle if memo was already scheduled or old input
    @if ($memo->status === 'scheduled' || old('action') === 'schedule' || old('scheduled_at') || $memo->scheduled_at)
