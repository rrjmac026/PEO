...
@section('content')
    ...
    <div class="max-w-2xl">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            ...
            <div class="px-6 md:px-8 py-6 bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-900/20 dark:to-blue-900/20 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center gap-4">
                    <div class="h-12 w-12 rounded-full bg-indigo-600 dark:bg-indigo-500 flex items-center justify-center">
                        <span class="text-white font-semibold text-lg">{{ substr($employee->user->name, 0, 1) }}</span>
                    </div>
                    <div>
                        <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ $employee->user->name }}</h2>
                        <p class="text-gray-600 dark:text-gray-400 text-sm">{{ $employee->employee_id }}</p>
                    </div>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="user_id" id="user_id" required 
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" @selected($employee->user_id == $user->id)>
